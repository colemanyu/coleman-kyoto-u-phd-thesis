...
Time series are sequences of data points measured at discrete timestamps. 
\begin{definition}[Time Series] 
    \label{def:ts}
    A time series $T = t_1, t_2, ..., t_n$ is a sequence of real-valued numbers with length = $n$.
\end{definition}
They are ubiquitous across domains such as finance, healthcare, meteorology, and engineering. 

Sometimes, we analyze the time series through its subsequences.
...
Many problems can be defined on time series data.
Several of them are listed below.
\begin{itemize}
    \item \textbf{Classification}: Assigning a predefined label/category to a time series. It will be further discussed in Chapter~\ref{ch:mtsccleav}.
    \item \textbf{Clustering}: Grouping a set of unlabeled time series based on their similarity.
    ...
    \item \textbf{Forecasting}: Given historical time series data (i.e., training set), predicting future values, which form a horizon. It will be further discussed in Chapter~\ref{ch:mpmf}.
...
In this chapter, we provide background on time series analysis.



with a focus on distance measures and classification, particularly the ridge classifier, which is used in the ROCKET-based classifiers for time series classification. 
...
\subsection{Calibrating the raw sequences}

\subsection{From whole sequence to subsequence}

\section{Subsequence analysis}

% Draw a figure for matrix profile
...
% Mention a bit about MPDist
% CID: Compensate for the disadvantage because of the complexity
% DTW prefix
% psdtw
...
% Transformation: Change time series into a set of features.
% Dicer cleavage site, subsequence
