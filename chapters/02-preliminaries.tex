...
%
Some tasks defined on time series are also introduced.
%
...
In detail, we focus on a powerful time-series data-mining primitive, namely the Matrix Profile, which complemetns Chapter~\ref{ch:mpmf}.
%
Finally, we introduce methods for transforming time series into tabular representations in Section~\ref{sec:transformations}, thereby enabling the use of other methods that were not originally designed for time series, which complements Chapter~\ref{ch:mpmf}.
...
\begin{itemize}
    ...
    \item \textbf{Forecasting}: Given historical time series data (i.e., training set), predicting future values (horizon). It will be further discussed in Chapter~\ref{ch:mpmf}.
...
\begin{equation}
    dtw(i,j) = \begin{cases} \infty, & \text{if } (i = 0 \text{ and } j > r) \text{ or } (j = 0 \text{ and } i > r) \\ 0, & \text{if } (i = 0 \text{ and } j \le r) \text{ or } (j = 0 \text{ and } i \le r) \end{cases}
\end{equation}
...
We have another vector, namely the Join Matrix Profile $P_{ABBA}$ to store the corresponding z-normalized ED of the 2-tuples stored in  $J_{ABBA}$.
...
\section{Transformations for feature extraction} \label{sec:transformations}
