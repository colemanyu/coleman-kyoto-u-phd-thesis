...
In detail, we focus on a powerful time-series data-mining primitive, namely Matrix Profile, which complements Chapter~\ref{ch:mpmf}.
...
Matrix Profile~\cite{zhu2020swiss} is a data mining primitive that can solve many tasks by computing the nearest neighbor of each $m$-subsequence in $T_A$. To note, those nearest neighbors could be found within the same time series (i.e., $T_A$) or from other time series (i.e., $T_B$). 
...
Any part of the distance profile that is smaller than $\theta$ points to a member of $M$ in $T$.
...
\section{Time series classification} \label{sec:tsc} % *!
