...
The $k$-NN uses the $w-1$ lagged values of the last time point to form a query $Q$ with length $w$.
It identifies the $k$ previous similar subsequences to $Q$ and uses their immediate subsequences to predict the immediate subsequence, which is the forecasting window of length $h$, of $Q$.
...
We also refer to the left gray box as the left nearest neighbor of the right gray box.

\input{03-matrix-profile-motif-forecasting-paper/algorithms/mp-brute-force}
Based on these findings, this study proposes a method to improve the performance of existing forecasters by leveraging information from the nearest neighbors of each subsequence in the target variable $Y$.
For each time point $y_i$ of the target variable $Y$, a window of length $w$ is constructed with $y_i$ as the last point, then we retrieve the window's left nearest neighbors and use their information to create new covariates for the window.
The information includes the similarities between the window and its nearest neighbors, as well as the immediate subsequences of those neighbors.
...
We use the Matrix Profile~\cite{yeh2016matrix, zhu2017matrix} to annotate the left nearest neighbor for each $m$-subsequence of a time series $T$ of length $n$.
The distance is the z-normalized Euclidean distance, which calculates the Euclidean distance between the two time series after they have been z-normalized to have a mean of zero and a standard deviation of one.
For a time series $T$, the resulting z-normalized $T'$ would have entries $t'_i = (t_i - \mu(T))/\sigma(T)$, where $\mu(T)$ and $\sigma(T)$ are the mean and the standard deviation, of $T$ respectively.
...
The tool also allows us to compute the matrix profile to find the nearest neighbor of each window in an efficient way.
Recall that we are interested in the ``left'' version of the matrix profile. 
...
\begin{itemize}
  ...
  \item We have proposed two methods namely, GBRT-NN, which uses the immediate subsequence as the covariates, and GBRT-NN-S, which also involves the similarity on top of GBRT-NN, as the covariates.
  \item Experimental results show that our approach can improve the GBRT forecaster in most cases, with a very small cost to compute the matrix profile.
\end{itemize}
