...
Consequently, retrieving such patterns requires assigning different scaling factors to each phase to accommodate the phase-specific rates (i.e., piecewise scaling distortions).
...
\subsubsection{Results of GunPoint dataset (train split)}
...
The results indicate that \texttt{vanilla\_PSDTW} is orders of magnitude slower than the other approaches.
...
We report the time proportion of lower bound calculation in Table~\ref{tab:time_proportion_lb}, which shows that the time proportion of the lower bound calculation for PSED is much higher than that of PSDTW.
...
An incorrect matching of $P$ between the query set and the target set would significantly degrade accuracy.
