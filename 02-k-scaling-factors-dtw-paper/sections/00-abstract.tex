...
%
% Motivation
Surprisingly, all existing studies use a single scaling factor across the entire time series.
A time series could consist of phases. 
Since each phase expresses at its own rate, using a single scaling factor is insufficient when comparing two time series that share similar phases but differ in expression rate.
%
% Contribution
We introduce the first framework that accounts for multiple scaling factors, Piecewise Scaling Distance (PSD).
It employs other distance measures as a subroutine.
...
Two additional speedup techniques have been proposed, which achieve 10.10X to 191.46X speedup.
We also demonstrate the use of a lower bound when DTW is used as a subroutine to speed up the computation.
