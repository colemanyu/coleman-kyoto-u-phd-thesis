\begin{algorithm*}[t] % *!
\scriptsize
\caption{Constrainted PSDTW with parallel computing, early abandoning and lower bounding}
...
\For{$p \gets 1$ \textbf{to} $P$}
    \color{blue}
    \For{$i \gets (p \cdot L_{\mathrm{gmin}}^Q)$ \textbf{to} $\min(p \cdot L_{\mathrm{gmax}}^Q, m)$} \Comment{This iterations can be parallelized.}
    \color{black}
        {\color{black}\For{$L^Q \gets L_{\mathrm{gmin}}^Q$ \textbf{to} $L_{\mathrm{gmax}}^Q$}
            ...
            \State $L_{\max}^C \gets \min(\floor{L^Ql}, L_{\mathrm{gmax}}^C)$
            % {\color{green}
            \color{green}
            \State $r' \gets r \times \max(L^Q, L_{\max}^C)$ \Comment{Compute the integer value of $r$.}
            \For{$k \gets 1$ \textbf{to} $L_{\max}^C$} 
                \State $\mathbbm{q}_k \gets (q'_{\max(1, \ceil{k/l} - r')}, \ldots, q'_{\min(\floor{kl} + r', L^Q)})$ \Comment{Construct indexed collection $\mathbbm{Q}$ of $Q'$.}
                \State $\tilde{\mathbbm{q}}_k \gets \operatorname{sort}(\mathbbm{q}_k)$
            \EndFor
            \color{black}
            % }
            \For{$j \gets (p \cdot L_{\mathrm{min}}^C$) \textbf{to} $\min(p \cdot L_{\mathrm{max}}^C, n)$} 
                \State $\mathit{lb} \gets 0.0$, $\mathit{is\_lb\_initialized} \gets \textsc{False}$
                \For{$L^C \gets L_{\min}^C$ \textbf{to} $L_{\max}^C$}
                    ...
                    \EndIf
                    \color{orange}
                    \If{$\mathit{dist}_\mathrm{prev} > \mathrm{bsf}$} 
                        \State \textbf{continue}
                    \EndIf
                    \color{black}
                    ...
                    \If{$\neg \mathit{is\_lb\_initialized}$}
                        \For{$k \gets 1$ \textbf{to} $L^C$} \Comment{Compute the lower bound from sketch.}
                            \If{$k = 1$} \Comment{Partial distance contributed by the first alignment.}
                                \State $\mathit{lb} = (c'_1 - q'_1)^2$ 
                            \Else
                                \State $\mathit{lb} = \mathit{lb} + \delta(c'_k, \tilde{\mathbbm{q}}_k)$
                            \EndIf
                            \If{$\mathit{lb} > D[i,j,p]$}
                                \State \textbf{break}
                            \EndIf
                            \State $\mathit{is\_lb\_initialized} \gets \textsc{True}$
                        \EndFor
                    \Else
                    ...
                    \State $\mathit{dist}_{\mathrm{seg}} \gets \operatorname{DTW}_r(Q'^L, C'^L$)                    \If{$\mathit{dist}_{\mathrm{prev}}+\mathit{dist}_{\mathrm{seg}} < D[i,j,p]$}
                        \State $D[i,j,p] \gets \mathit{dist}_{\mathrm{prev}}+\mathit{dist}_{\mathrm{seg}}$ \Comment{Also save the 2-tuple $(i', j')$ for the cut.}
                    \EndIf
                \EndFor
            \EndFor
        \EndFor}
